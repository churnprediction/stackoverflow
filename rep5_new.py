...
def cdfRep(users):
    title("Reputation of user after 5 posts vs user count")
    xlabel("Reputation")
...
QRY = "select t2.uid, t2.posttypeid, votetypeid, count(votetypeid) from votes force index(idx_votes_postid_votetypeid), (select uid, posttypeid, id from posts force index(idx_owneruserid_postid), user5  where posts.owneruserid = user5.uid and posts.id <= user5.post5id) t2 where t2.id = votes.postid and votes.votetypeid in(1, 2, 3) group by t2.uid, t2.posttypeid, votetypeid"

res = getRes(QRY, True, 'user5_res')
