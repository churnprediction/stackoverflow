...
for clf in [ (DecisionTreeClassifier(), 'DT'), (SVC(), 'SVC') ]:

    for K in itertools.chain(range(20, 15, -1), range(5, 0, -1)):
        print "K: ",K
        X, y = getXyFromCsv('data/y_X_churn_K_' + str(K) + '.csv', fillMissingWith0=True)
        print "Number of features", shape(X)
        print clf
        print performClassification(X, y, clf[0], clf[1], False, scaleX=True)
