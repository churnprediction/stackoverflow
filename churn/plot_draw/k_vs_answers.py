...
title('K vs fraction of posts that are answers')
xlabel('Number of observation posts (K)')
ylabel('Fraction of posts that are answers')
fname='data/feat_avg_numanswer.csv'
...
Churn = X[X[:, 1] == 1]
NoChurn = X[X[:, 1] == 0]
pylab.plot(Churn[:, 0],  Churn[:, 2], 'd-', label='Churn', color='r')
pylab.plot(NoChurn[:, 0], NoChurn[:, 2], 'd-', label='Stay', color='g')
pylab.axhline(y=0.5, color='k', linestyle='dashed', label='50%')

#legend(prop={'size':'24'}, loc='lower right')
legend()
